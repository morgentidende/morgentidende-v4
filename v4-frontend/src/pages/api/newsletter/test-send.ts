...
const copenhagenDate = (date: Date) => new Intl.DateTimeFormat('en-CA', {
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  if (!expectedKey || !suppliedKey || !safeEqual(expectedKey, suppliedKey)) return reply(404, { ok: false, error: 'invalid_key' });

  const env = getNewsletterRuntimeEnv(locals);
  const supabase = getV4SupabaseServer(locals);
  if (!supabase || !hasSesEnv(env)) return reply(503, { ok: false, error: 'runtime_not_ready' });

  const subscriberResult = await supabase.rpc('newsletter_test_recipient');
  if (subscriberResult.error) return reply(500, { ok: false, error: 'subscriber_lookup_failed' });

  const subscribers = subscriberResult.data || [];
  if (subscribers.length !== 1) return reply(409, {
    ok: false,
  ...
  const now = new Date();
  const articleResult = await supabase.rpc('newsletter_daily_articles', { p_now: now.toISOString() });
  if (articleResult.error) return reply(500, { ok: false, error: 'article_lookup_failed' });
  ...
  if (!mail.ok) return reply(502, { ok: false, error: 'ses_failed', status: mail.status });
