...
function validatePayload(payload: Json) {
  ...
  if (
    payload.editorial_metadata != null &&
...
async function ingest(payload: Json) {
  ...
  if (!response.ok) {
    throw new Error(
      `rpc_${response.status}:${typeof parsed === "string" ? parsed.slice(0, 400) : JSON.stringify(parsed).slice(0, 400)}`,
    );
  }
  return parsed;
}
