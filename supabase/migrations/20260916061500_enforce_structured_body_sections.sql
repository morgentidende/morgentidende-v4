-- Kilder and Læs også are structured UI owned by source_metadata and article
-- relations. Keep body_markdown free of manual copies, using the same cheap
-- deterministic write/render invariant as SAGEN KORT.

create or replace function public.strip_structured_sections_from_markdown(p_body text)
returns text
language plpgsql
immutable
...
declare
  ...
  v_body text := public.strip_sagen_kort_from_markdown(coalesce(p_body, ''));
  v_next text;
  v_lines text[];
  ...
  v_line text;
begin
  -- Raw HTML form: exact canonical heading plus an immediately following list.
  v_next := regexp_replace(
    v_body,
    '(?is)<h([1-6])[^>]*>[[:space:]]*kilder[[:space:]]*</h\1>[[:space:]]*<(ul|ol)[^>]*>.*?</\2>',
    '',
    'g'
  );
  v_body := v_next;
  ...
  v_next := regexp_replace(
    v_body,
    '(?is)<h([1-6])[^>]*>[[:space:]]*(læs|laes)[[:space:]]+(også|ogsaa)[[:space:]]*</h\1>[[:space:]]*<(ul|ol)[^>]*>.*?</\4>',
    '',
    'g'
  );
  v_body := v_next;

  -- Also remove a standalone exact heading. Ordinary prose mentioning the words
  -- is deliberately not matched.
  v_body := regexp_replace(
    v_body,
    '(?is)<h([1-6])[^>]*>[[:space:]]*kilder[[:space:]]*</h\1>',
    '',
    'g'
  );
  v_body := regexp_replace(
    v_body,
    '(?is)<h([1-6])[^>]*>[[:space:]]*(læs|laes)[[:space:]]+(også|ogsaa)[[:space:]]*</h\1>',
    '',
    'g'
  );
  ...
  while v_i <= v_upper loop
    ...
    if v_line ~* '^[[:space:]]{0,3}(#{1,6}[[:space:]]*)?(kilder|(læs|laes)[[:space:]]+(også|ogsaa))[[:space:]]*#*[[:space:]]*$' then
      v_cursor := v_i + 1;
  ...
  if v_body is not distinct from v_original then
    return v_original;
  end if;
  return v_body;
end;
...
begin
  ...
  v_normalized := public.strip_structured_sections_from_markdown(v_body);

  if v_normalized is distinct from v_body then
    raise log 'article_id=%, event=duplicate_structured_section_removed, source_stage=pre_publish', new.id;
  end if;
...
begin
  ...
  if not p_check_duplicate then return null; end if;
