-- Fail closed on magazine dedupe: every new magazine payload needs a stable topic key.
-- Existing queue-id/slug retries remain idempotent before the new requirement is evaluated.
...
begin
  ...
  -- Preserve retries for already-created transport jobs, including legacy rows.
  ...
  if v_kind = 'magazine'::public.article_kind then
    ...
    -- Persist the normalized/defaulted contract that downstream triggers inspect.
