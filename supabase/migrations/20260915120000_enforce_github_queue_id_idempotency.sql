-- Make the GitHub transport idempotency key a DB-level invariant.
-- Historical/manual rows legitimately have no queue id, so the column remains nullable.

alter table public.articles
  add column if not exists github_queue_id text
  generated always as (nullif(editorial_metadata->>'github_queue_id', '')) stored;

create unique index if not exists articles_github_queue_id_uq
  on public.articles (github_queue_id)
  where github_queue_id is not null;
...
begin
  ...
  where lower(slug) = v_category_slug
    and active = true
  limit 1;
  ...
  -- Serialize retries for the same transport key before any idempotency lookup.
  -- The partial UNIQUE index remains the ultimate invariant.
  perform pg_advisory_xact_lock(hashtextextended('github_queue_id:' || v_queue_id, 0));

  select id into v_article_id
  from public.articles
  where github_queue_id = v_queue_id
  ...
  insert into public.articles (
    slug, kind, status, category_id, story_cluster_id, headline,
    frontpage_headline, headline_accent_text, deck, body_markdown,
    author_name, author_title, hero_url, hero_alt, hero_source_url,
    hero_candidate_url, hero_candidate_note, hero_credit, hero_license,
    hero_license_url, is_lead, lead_rank, is_breaking, breaking_until,
    publish_at, created_by, source_metadata, editorial_metadata, topic_key
  ) values (
    v_slug, v_kind, 'scheduled'::public.article_status, v_category_id,
    v_story_cluster_id, v_headline, nullif(p_payload->>'frontpage_headline', ''),
    nullif(p_payload->>'headline_accent_text', ''), nullif(p_payload->>'deck', ''),
    v_body, nullif(p_payload->>'author_name', ''), nullif(p_payload->>'author_title', ''),
    nullif(p_payload->>'hero_url', ''), nullif(p_payload->>'hero_alt', ''),
    nullif(p_payload->>'hero_source_url', ''), nullif(p_payload->>'hero_candidate_url', ''),
    nullif(p_payload->>'hero_candidate_note', ''), nullif(p_payload->>'hero_credit', ''),
    nullif(p_payload->>'hero_license', ''), nullif(p_payload->>'hero_license_url', ''),
    coalesce((p_payload->>'is_lead')::boolean, false), nullif(p_payload->>'lead_rank', '')::integer,
    coalesce((p_payload->>'is_breaking')::boolean, false),
    nullif(p_payload->>'breaking_until', '')::timestamptz, v_publish_at,
    'chatgpt_scheduled_github_bridge', v_source_metadata, v_editorial_metadata, v_topic_key
  )
